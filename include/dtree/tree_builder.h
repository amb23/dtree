...
template <typename splitter_t, typename cost_fn_t> class tree_builder {
public:
    ...
    tree_builder(
        const tree_builder_config& config, splitter_t splitter, cost_fn_t cost_fn)
        : m_config { config }
    ...
    tree_t build(const feature_set& features, const labels& labels_,
...
private:
    ...
    void build(tree_t& tree, std::size_t loc, const feature_set& features,
        const labels& labels_, const stop_condition& stop_condition) const
    {
        if (auto reason = stop_condition.Check(tree_t::get_depth(loc), labels_)) {
            SPDLOG_DEBUG(
                "Stopping building at depth {} as {}", depth, to_string(*reason));
            tree[loc] = leaf { labels_.calculate_distribution() };
        } else {
            auto split = m_splitter.split(features, labels_, m_cost_fn);
            SPDLOG_DEBUG("Found optimal cut {} for feature {} with cost {}", cut,
                feature_id, cost);

            tree[loc] = split;

            build_sub_trees(tree, loc, features, labels_, stop_condition, split);
        }
    }

    template <typename feature_set, typename split_t>
    void build_sub_trees(tree_t& tree, std::size_t loc, const feature_set& features,
        const labels& labels_, const stop_condition& stop_condition,
    ...
    void build_sub_trees_async(tree_t& tree, std::size_t loc,
        const feature_set& features, const labels& labels_,
        const stop_condition& stop_condition, const split_t& split) const
    {
        auto lower_future = std::async(
            [this, &tree, &features, &labels_, &stop_condition, split, loc]() {
                return build_sub_tree(
                    tree, loc, true, features, labels_, stop_condition, split);
            });

        auto upper_future = std::async(
            [this, &tree, &features, &labels_, &stop_condition, split, loc]() {
                return build_sub_tree(
                    tree, loc, false, features, labels_, stop_condition, split);
            });
    ...
    void build_sub_trees_sync(tree_t& tree, std::size_t loc, const feature_set& features,
        ...
        const split_t& split) const
    {
        build_sub_tree(tree, loc, true, features, labels_, stop_condition, split);

        build_sub_tree(tree, loc, false, features, labels_, stop_condition, split);
    }
        ...
        const stop_condition& stop_condition, const split_t& split) const
    {
        ...
        const auto& split_index
            = index_splitter_.get_splitting_index(lower, features, split);
        ...
        for (const auto& [feature_id, feature] : features) {
            ...
            feature_t split_feature(begin(split_view), end(split_view));
